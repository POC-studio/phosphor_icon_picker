const e=`export default function(instance, context) {
  ...
  // L'icône par défaut sera définie dans update.js via les properties,
  // on initialise avec une icône vide temporairement.
  ...
  mainIcon.style.lineHeight = '1'; // Évite le recadrage vertical causé par l'interligne de la police
  ...
  // 4. Créer le menu déroulant (dropdown)
  const dropdown = document.createElement('div');
  dropdown.style.position = 'absolute';
  // On retire le top/left fixes puisqu'ils seront calculés dynamiquement
  ...
  searchInput.style.boxSizing = 'border-box'; // Empêche la barre de déborder du dropdown
  ...
  // Filtrer les icônes à la saisie
  ...
  // Remplir le dropdown avec les icônes
  ICONS.forEach(iconName => {
    const iconBtn = document.createElement('i');
    // On ajoute le préfixe 'ph-' uniquement pour l'affichage via la librairie Phosphor
    iconBtn.className = instance.data.getIconClass(iconName, instance.data.currentStyle);
    iconBtn.dataset.iconName = iconName;
    instance.data.dropdownIcons.push(iconBtn);
    
    // On garde une taille fixe pour le menu déroulant pour plus de lisibilité
    iconBtn.style.fontSize = '24px';
    iconBtn.style.padding = '8px';
    iconBtn.style.cursor = 'pointer';
    iconBtn.style.borderRadius = '4px';
    iconBtn.style.color = instance.data.currentColor;
    ...
    // Au clic sur une icône dans le dropdown
    iconBtn.addEventListener('click', (e) => {
      e.stopPropagation(); // Empêcher la propagation pour ne pas refermer immédiatement
      ...
      // Fermer le dropdown et mettre à jour le state
      dropdown.style.display = 'none';
      instance.publishState('is_open', false);
    });
    
    iconBtn.title = iconName; // Tooltip avec le nom de l'icône
    iconsGrid.appendChild(iconBtn);
  });

  dropdown.appendChild(iconsGrid);
  document.body.appendChild(dropdown); // On l'attache au body pour éviter qu'il soit coupé par un overflow hidden de Bubble

  // 5. Gérer l'ouverture / fermeture du dropdown
  mainIcon.addEventListener('click', (e) => {
    ...
    if (isCurrentlyClosed) {
      ...
      // Placer le dropdown juste en dessous
  ...
  // 6. Attacher le tout au canvas Bubble
  instance.canvas.append(container);
  
  // Optionnel : Sauvegarder la référence pour update.js plus tard
