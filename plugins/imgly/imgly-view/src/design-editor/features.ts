...
import type { CreativeEditorSDK, FeaturePredicateContext } from '@cesdk/cesdk-js';

const PAGE_BLOCK_TYPE = '//ly.img.ubq/page';

function isPageBlockSelected(engine: FeaturePredicateContext['engine']): boolean {
  return engine.block.findAllSelected().some(
    (blockId) => engine.block.getType(blockId) === PAGE_BLOCK_TYPE,
  );
}

/** Masque toute interaction UI quand une page (canvas) est sélectionnée. */
function hideWhenPageSelected({
  isPreviousEnable,
  engine,
}: FeaturePredicateContext): boolean {
  if (!isPreviousEnable()) return false;
  return !isPageBlockSelected(engine);
}

/** Features d’apparence de page à verrouiller (couleur, contour, effets…). */
const PAGE_APPEARANCE_FEATURES = [
  'ly.img.fill',
  'ly.img.fill.*',
  'ly.img.stroke',
  'ly.img.stroke.*',
...
/** Menus / barres / actions interdits sur une page sélectionnée. */
const PAGE_UI_FEATURES = [
  'ly.img.duplicate',
  'ly.img.canvas.menu',
